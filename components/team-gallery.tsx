...
function GalleryViewer({ initialIndex, onClose }: { initialIndex: number; onClose: () => void }) {
  ...
  return (
    <dialog ref={dialogRef} aria-label="Fileredge photo gallery" onCancel={onClose} onClick={(event) => { if (event.target === event.currentTarget) onClose(); }} onKeyDown={(event) => { if (event.key === "ArrowLeft") { event.preventDefault(); previous(); } if (event.key === "ArrowRight") { event.preventDefault(); next(); } }} className="fixed inset-0 m-auto w-[calc(100%_-_24px)] max-w-[1200px] overflow-hidden rounded-2xl border border-white/15 bg-navy p-4 text-white shadow-2xl backdrop:bg-[#020b19]/90 sm:p-6">
      ...
      <div className="relative h-[min(60dvh,calc(100dvh_-_190px))] sm:h-[min(65dvh,calc(100dvh_-_210px))]"><Image src={photo.src} alt={photo.alt} fill sizes="(max-width: 1200px) 95vw, 1150px" className="object-contain" /></div>
