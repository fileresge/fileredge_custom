...
export default function BlogLibrary() {
  ...
  return (
    <section id="articles" aria-labelledby="articles-heading" className="scroll-mt-8 py-16 sm:py-20">
      <div className="mb-7 flex flex-col justify-between gap-5 md:flex-row md:items-center"><h2 id="articles-heading" className="text-3xl font-extrabold tracking-[-0.8px] text-navy">Explore the journal</h2><div className="relative w-full md:w-[320px]"><label htmlFor="blog-search" className="sr-only">Search articles</label><Search size={18} className="pointer-events-none absolute top-1/2 left-4 -translate-y-1/2 text-[#8a8f99]" aria-hidden="true" /><input id="blog-search" type="search" placeholder="Find an article..." value={query} onChange={(event) => setQuery(event.target.value)} className="w-full rounded-full border border-[#e1e4e9] bg-white py-3.5 pr-11 pl-11 text-sm outline-none focus:border-brand-orange focus:ring-2 focus:ring-brand-orange/10 [&::-webkit-search-cancel-button]:appearance-none" />{query && <button type="button" aria-label="Clear article search" onClick={() => setQuery("")} className="absolute top-1/2 right-3 -translate-y-1/2 rounded-full p-1 text-[#7b818e]"><X size={17} /></button>}</div></div>
